...
router.get("/dashboard", auth, async (req, res) => {
  try {
    ...
    if (req.user.role === "admin") {
      ...
      const [jumlahSiswa] = await db.query(
        `SELECT COUNT(DISTINCT user_id) as total FROM siswa ${taWhere}`,
        taParams,
      );
      const [jumlahGuru] = await db.query(
        "SELECT COUNT(*) as total FROM guru g JOIN users u ON g.user_id = u.id WHERE u.is_active = 1",
      );
      const [jumlahKelas] = await db.query(
        `SELECT COUNT(*) as total FROM kelas ${taWhere}`,
        taParams,
      );
      ...
      const [absensiSummary] = await db.query(
        ...
        FROM absensi_harian ah ${ahWhere}`,
        taParams,
      );
    ...
    if (req.user.role === "guru") {
      const [g] = await db.query(
        "SELECT id, is_walas, kelas_id FROM guru WHERE user_id = ?",
        [req.user.id],
      );
      ...
      const [jadwals] = await db.query(
        `SELECT j.id, m.nama as mapel, k.nama as kelas, j.hari, j.jam_mulai, j.jam_selesai, j.tipe_sesi, j.pola_minggu FROM jadwal_mengajar j
        JOIN mapel m ON j.mapel_id = m.id JOIN kelas k ON j.kelas_id = k.id
        WHERE j.guru_id = ? ${taWhere} ORDER BY FIELD(j.hari,'Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'), j.jam_mulai`,
        taParams,
      );
      ...
      if (guru.is_walas && guru.kelas_id) {
        ...
        const [ks] = await db.query(
          ...
          SELECT SUM(status='hadir') as hadir, SUM(status='sakit') as sakit,
                 SUM(status='izin') as izin, SUM(status='alpha') as alpha, COUNT(*) as total
          FROM absensi_harian ah WHERE ah.kelas_id = ? ${kwhere}`,
          kparams,
        );
        kelasStats = ks[0];
      }
      ...
      let totalSiswa = 0,
        kelasNama = null;
      if (guru.is_walas && guru.kelas_id) {
        const [kInfo] = await db.query(
          "SELECT k.nama, COUNT(s.id) as cnt FROM kelas k LEFT JOIN siswa s ON s.kelas_id = k.id LEFT JOIN users us ON s.user_id = us.id AND us.is_active=1 WHERE k.id = ? GROUP BY k.id",
          [guru.kelas_id],
        );
        if (kInfo.length) {
          totalSiswa = kInfo[0].cnt;
          kelasNama = kInfo[0].nama;
        }
      }
      return res.json({
        success: true,
    ...
    if (req.user.role === "siswa") {
      const [s] = await db.query(
        "SELECT s.id, s.kelas_id FROM siswa s JOIN tahun_ajaran ta ON s.tahun_ajaran_id = ta.id WHERE s.user_id = ? ORDER BY s.id DESC LIMIT 1",
        [req.user.id],
      );
      ...
      const [stats] = await db.query(
        ...
        SELECT SUM(status='hadir') as hadir, SUM(status='sakit') as sakit,
               SUM(status='izin') as izin, SUM(status='alpha') as alpha, COUNT(*) as total
        FROM absensi_harian ah WHERE ah.siswa_id = ? ${taWhere}`,
        taParams,
      );
      ...
      const [jadwals] = await db.query(
        `
        SELECT j.id, m.nama as mapel, k.nama as kelas, j.hari, j.jam_mulai, j.jam_selesai, j.tipe_sesi, j.pola_minggu, u.nama as guru_nama
        FROM jadwal_mengajar j
        JOIN mapel m ON j.mapel_id = m.id 
        JOIN kelas k ON j.kelas_id = k.id
        JOIN guru g ON j.guru_id = g.id
        JOIN users u ON g.user_id = u.id
        WHERE j.kelas_id = ? ${taFilter ? "AND j.tahun_ajaran_id = ?" : ""}
        ORDER BY FIELD(j.hari,'Senin','Selasa','Rabu','Kamis','Jumat','Sabtu'), j.jam_mulai`,
        taFilter ? [s[0].kelas_id, taFilter] : [s[0].kelas_id],
      );

      return res.json({
        success: true,
        data: { stats: stats[0], monthly, jadwals },
      });
    }
