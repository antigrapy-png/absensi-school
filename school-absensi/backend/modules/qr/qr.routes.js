  ...
  async (req, res) => {
    try {
      ...
      if (!jadwal_id || !tanggal)
        return res
          .status(400)
          .json({ success: false, message: "jadwal_id dan tanggal wajib" });
      ...
      await db.query(
        "UPDATE qr_sesi SET is_active=0 WHERE jadwal_id=? AND tanggal=?",
        [jadwal_id, tanggal],
      );
      ...
      await logActivity(
        req.user.id,
        "QR_GENERATE",
        `QR sesi jadwal #${jadwal_id} tgl ${tanggal}`,
        req.ip,
      );
      res.json({
        success: true,
        data: {
          ...
          durasi_menit: durasiMin,
          kelas_id: j[0].kelas_id,
        },
      });
    } catch (err) {
...
router.post("/scan", auth, async (req, res) => {
  try {
    if (req.user.role !== "siswa")
      return res
        .status(403)
        .json({ success: false, message: "Hanya siswa yang bisa scan QR" });
    ...
    if (sw[0].kelas_id !== s.kelas_id) {
      return res
        .status(403)
        .json({ success: false, message: "QR Code bukan untuk kelas Anda" });
    }

    // Cek sudah scan
    const [existing] = await db.query(
      "SELECT id FROM qr_scan_log WHERE qr_sesi_id=? AND siswa_id=?",
      [s.id, sw[0].id],
    );
    if (existing.length)
      return res
    ...
    // Insert scan log
    await db.query(
      "INSERT INTO qr_scan_log (qr_sesi_id, siswa_id, status) VALUES (?,?,?)",
      [s.id, sw[0].id, scanStatus],
    );

    // Update absensi mapel
    const [jadwal] = await db.query(
      "SELECT * FROM jadwal_mengajar WHERE id=?",
      [s.jadwal_id],
    );
    if (jadwal.length) {
      await db.query(
        ...
        [
          ...
          sw[0].id,
          s.tanggal,
          jadwal[0].tahun_ajaran_id,
          scanStatus === "terlambat" ? "hadir" : "hadir",
          sw[0].id,
        ],
      );
    }
    ...
    await logActivity(
      req.user.id,
      "QR_SCAN",
      `Scan QR sesi #${s.id} status: ${scanStatus}`,
      req.ip,
    );
    res.json({
      success: true,
      message: `Absensi berhasil! Status: ${scanStatus === "terlambat" ? "Hadir (Terlambat)" : "Hadir"}`,
...
router.get("/sesi-aktif", auth, async (req, res) => {
  try {
    ...
    const [sw] = await db.query(
      "SELECT kelas_id FROM siswa WHERE user_id = ? ORDER BY id DESC LIMIT 1",
      [req.user.id],
    );
...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      ORDER BY qsl.scanned_at ASC`,
        [req.params.sesi_id],
      );

      const [sesi] = await db.query("SELECT * FROM qr_sesi WHERE id=?", [
        req.params.sesi_id,
      ]);
      const [allSiswa] = await db.query(
        "SELECT s.id, u.nama, s.nis FROM siswa s JOIN users u ON s.user_id=u.id WHERE s.kelas_id=? AND u.is_active=1",
        [sesi[0]?.kelas_id],
      );
      ...
      res.json({
        success: true,
        data: { scanned: rows, belumScan, sesi: sesi[0] },
      });
    } catch (err) {
      res.status(500).json({ success: false, message: err.message });
    }
...
router.delete(
  ...
  async (req, res) => {
    try {
      await db.query("UPDATE qr_sesi SET is_active=0 WHERE id=?", [
        req.params.id,
      ]);
